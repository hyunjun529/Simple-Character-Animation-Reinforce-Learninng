...
struct LabF203 : public CommonRigidBodyBase
{
	...
	const int initStep = 50;
	int cntStep = 0;
	const int howManyRepeat = 3;
	int cntResetTarget = 0;


	LabF203(struct GUIHelperInterface* helper);
	virtual ~LabF203();
...
LabF203::LabF203(struct GUIHelperInterface* helper)
...
void LabF203::lockLiftHinge(btHingeConstraint* hinge)
{
...
void LabF203::stepSimulation(float deltaTime)
{
	if (cntStep < initStep) {
		cntStep++;
	...
	if (collisionTarget) {
		...
		if (cntResetTarget < howManyRepeat) {
			initTarget();
			cntResetTarget++;
		}
		else {
...
void LabF203::initPhysics()
{
	...
	for (int i = 0; i<numLinks; i++)
	{
		...
		if (i == 0)
		{
			...
			//hinge_shoulder->setLimit(M_PI / 0.42f, M_PI / 0.42f);
...
bool LabF203::keyboardCallback(int key, int state)
{
	bool handled = true;
	if (state)
	{
...
CommonExampleInterface*    env_203(CommonExampleOptions& options)
{
	return new LabF203(options.m_guiHelper);
}