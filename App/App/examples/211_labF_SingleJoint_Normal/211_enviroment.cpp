...
struct LabF211 : public CommonRigidBodyBase
{
	...
	LabF211(struct GUIHelperInterface* helper);
	virtual ~LabF211();
...
LabF211::LabF211(struct GUIHelperInterface* helper)
...
void LabF211::stepSimulation(float deltaTime)
{
	/***************************************************************************************************/
...
bool LabF211::keyboardCallback(int key, int state)
{
	bool handled = true;
	if (state)
	{
...
CommonExampleInterface*    env_211(CommonExampleOptions& options)
{
	return new LabF211(options.m_guiHelper);
}