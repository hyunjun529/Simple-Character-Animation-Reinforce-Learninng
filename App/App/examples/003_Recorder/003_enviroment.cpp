...
struct Recorder : public CommonRigidBodyBase
{
	...
	void initState(Recorder* target) {
...
Recorder::Recorder(struct GUIHelperInterface* helper)
...
void Recorder::stepSimulation(float deltaTime)
{
	//get distance
...
bool Recorder::keyboardCallback(int key, int state)
{
	bool handled = true;
	if (state)
	{
...
CommonExampleInterface*    env_003(CommonExampleOptions& options)
{
	return new Recorder(options.m_guiHelper);
}