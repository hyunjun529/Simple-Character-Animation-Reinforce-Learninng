...
struct BodyNNTest : public CommonRigidBodyBase
{
	bool m_once;
	...
	ArmReinforcementLearning rl_sd_030_;
	ArmReinforcementLearning rl_eb_030_;

	BodyNNTest(struct GUIHelperInterface* helper);
	virtual ~BodyNNTest();
	...
	void initState(BodyNNTest* target) {
...
BodyNNTest::BodyNNTest(struct GUIHelperInterface* helper)
	...
	m_once(true)
{
	...
	*********************************************************************************************/
	...
	// h529 : 이전 게임의 기억을 가져감
	rl_sd_030_.num_input_histories_ = 1;
	// h529 : 기억을 재생함, pool로 사용 중
	rl_sd_030_.num_exp_replay_ = 0;
	// h529 : eb_angle, sd_angle, F2T_distance, F2T_angle
	rl_sd_030_.num_state_variables_ = 4;
	// h529 : 행동할 수 있는 Action의 개수 현재 좌, 우 2개
	rl_sd_030_.num_game_actions_ = 3;

	rl_sd_030_.initialize();

	for (int h = 0; h < rl_sd_030_.num_input_histories_; h++)
	{
		rl_sd_030_.recordHistory(VectorND<float>(4), 0.0f, 2, VectorND<float>(3)); // choice 2 is stay
	}

	// h529 : 이전 게임의 기억을 가져감
	rl_eb_030_.num_input_histories_ = 1;
	// h529 : 기억을 재생함, pool로 사용 중
	rl_eb_030_.num_exp_replay_ = 0;
	// h529 : eb_angle, sd_angle, F2T_distance, F2T_angle
	rl_eb_030_.num_state_variables_ = 4;
	// h529 : 행동할 수 있는 Action의 개수 현재 좌, 우 2개
	rl_eb_030_.num_game_actions_ = 3;

	rl_eb_030_.initialize();

	for (int h = 0; h < rl_eb_030_.num_input_histories_; h++)
	{
		rl_eb_030_.recordHistory(VectorND<float>(4), 0.0f, 2, VectorND<float>(3)); // choice 2 is stay
	}
...
void BodyNNTest::lockLiftHinge(btHingeConstraint* hinge)
{
...
void BodyNNTest::stepSimulation(float deltaTime)
{
	...
	*********************************************************************************************/
	...
	rl_sd_030_.nn_.copyOutputVectorTo(false, output_vector_temp);
	VectorND<float> output_target_temp;

	rl_eb_030_.forward();
	VectorND<float> output_vector_temp_eb;
	rl_eb_030_.nn_.copyOutputVectorTo(false, output_vector_temp_eb);
	VectorND<float> output_target_temp_eb;

	// set Action
	float dice = (chkStudying) ? (0.3f) : (0.0f);
	int action_sd = rl_sd_030_.nn_.getOutputIXEpsilonGreedy(dice);
	int action_eb = rl_eb_030_.nn_.getOutputIXEpsilonGreedy(dice);
	...
	*********************************************************************************************/
	...
	rl_sd_030_.recordHistory(state_, reward_, action_sd, output_vector_temp);
	rl_eb_030_.recordHistory(state_, reward_, action_eb, output_vector_temp_eb);
	...
	*********************************************************************************************/
	...
	if ((chkCollision || cntStep > maxStep) && chkStudying) {
		...
		// compensation
		for (float &val : rl_sd_030_.memory_.reward_array_) {
			val *= weightStepEarly;
		}
		for (float &val : rl_eb_030_.memory_.reward_array_) {
			val *= weightStepEarly;
		}
		...
		for (int tr = 0; tr < tr_num; tr++)
			for (int m_tr = rl_sd_030_.memory_.num_elements_ - 2; m_tr >= rl_sd_030_.num_input_histories_; m_tr--)
			{
				// h529 : 방학숙제에서 발췌
				...
				int m = rand() % (rl_sd_030_.memory_.num_elements_ - 1 - rl_sd_030_.num_input_histories_) + rl_sd_030_.num_input_histories_;

				// memory index from end
				const int inv_m = m - (rl_sd_030_.memory_.num_elements_ - 1);

				float Q_next = 0.0f;

				if (m != rl_sd_030_.memory_.num_elements_ - 2) // if next is not the terminal state
				{
					// Q_next = ...;
					Q_next = rl_sd_030_.memory_.q_values_array_[m + 1].getMaxValue();
				}

				float Q_target;
				// Q_target = ...;
				Q_target = Q_next + rl_sd_030_.memory_.reward_array_[m];

				// forward propagation from previous inputs
				rl_sd_030_.makeInputVectorFromHistory(inv_m - 1, rl_sd_030_.old_input_vector_);
				rl_sd_030_.nn_.setInputVector(rl_sd_030_.old_input_vector_);
				for (int i = 0; i < 100; i++)
				{
					rl_sd_030_.nn_.feedForward();
					rl_sd_030_.nn_.copyOutputVectorTo(false, output_target_temp);

					// output_target_temp[...] = ...;
					output_target_temp[rl_sd_030_.memory_.selected_array_[m]] = Q_target;

					rl_sd_030_.nn_.propBackward(output_target_temp);
				}

				rl_sd_030_.nn_.check();
			}

		// eb
		for (int tr = 0; tr < tr_num; tr++)
			for (int m_tr = rl_eb_030_.memory_.num_elements_ - 2; m_tr >= rl_eb_030_.num_input_histories_; m_tr--)
			{
				// h529 : 방학숙제에서 발췌
				...
				// h529 : 전체를 요약한 부분을 확률적으로 선택해서 학습하는 방법론
				// h529 : http://sanghyukchun.github.io/74/
				int m = rand() % (rl_eb_030_.memory_.num_elements_ - 1 - rl_eb_030_.num_input_histories_) + rl_eb_030_.num_input_histories_;

				// memory index from end
				const int inv_m = m - (rl_eb_030_.memory_.num_elements_ - 1);

				float Q_next = 0.0f;

				if (m != rl_eb_030_.memory_.num_elements_ - 2) // if next is not the terminal state
				{
					// Q_next = ...;
					Q_next = rl_eb_030_.memory_.q_values_array_[m + 1].getMaxValue();
				}

				float Q_target;
				// Q_target = ...;
				Q_target = Q_next + rl_eb_030_.memory_.reward_array_[m];

				// forward propagation from previous inputs
				rl_eb_030_.makeInputVectorFromHistory(inv_m - 1, rl_eb_030_.old_input_vector_);
				rl_eb_030_.nn_.setInputVector(rl_eb_030_.old_input_vector_);
				for (int i = 0; i < 100; i++)
				{
					rl_eb_030_.nn_.feedForward();
					rl_eb_030_.nn_.copyOutputVectorTo(false, output_target_temp_eb);

					// output_target_temp[...] = ...;
					output_target_temp_eb[rl_eb_030_.memory_.selected_array_[m]] = Q_target;

					rl_eb_030_.nn_.propBackward(output_target_temp_eb);
				}

				rl_eb_030_.nn_.check();
			}

		// reset
		rl_sd_030_.memory_.reset();
		rl_eb_030_.memory_.reset();
...
bool BodyNNTest::keyboardCallback(int key, int state)
{
	bool handled = true;
	if (state)
	{
...
CommonExampleInterface*    env_030(CommonExampleOptions& options)
{
	return new BodyNNTest(options.m_guiHelper);
}